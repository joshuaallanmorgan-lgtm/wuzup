...
v1 ships a good **UI + finder**. v2 makes it **intelligent**. This is mostly *invisible* work — no new screens — but it's what turns "a clean events directory" into "an app that knows what I'd actually want to do." Highest-leverage post-MVP investment.
...
- **Tinder / swipe UI redesign** — "the first thing that'll need to look different" once the engine is real. The v1 `TINDER.md` module ships first; v2 re-skins/expands it to match the smarter engine. (Tracked here, not in v1.)

## Open decisions (for later)
- How explicit vs implicit should tuning feel? (visible sliders vs silent behavioral learning vs both)
- Cold-start: what does a brand-new user get before there's signal?
- On-device model vs backend? (privacy contract pushes on-device)
- How much of "tie back to generation" is re-ranking the existing finder output vs. influencing the finder itself?

## Prior art already in the repo (don't reinvent)
- `taste.js` / TastePanel / CalibrationDeck / InterestEditor (the v1 thin version).
- PHASE_3.7 Addendum G — **Activity Evidence Layer** (the honest scoring mechanism; overlaps the ranking half).
- LONG_TERM "Event ranking & tiers (the infinite list)" + "Personalization" — folded into this spec.

## Relation to v1
v1 keeps the *thin* version (taste lightly reorders; swipe deck collects signal). Nothing here blocks launch. This spec is the **post-launch headline build**, developed early so it's ready when v1 ships.
