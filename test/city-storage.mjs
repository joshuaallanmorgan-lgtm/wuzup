...
test('storage listeners, profile identity, and session fatigue use canonical scopes', () => {
  ...
  const profileState = readFileSync(join(ROOT, 'app', 'src', 'profile-state.js'), 'utf8')
  ...
  assert.match(edit, /readProfileState/)
  assert.match(edit, /writeProfileState/)
  assert.match(profile, /readProfileState/)
  assert.match(profileState, /globalGet/)
  assert.match(profileState, /globalSet/)
